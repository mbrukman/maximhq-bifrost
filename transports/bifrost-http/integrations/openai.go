...
func CreateOpenAIListModelsRouteConfigs(pathPrefix string, handlerStore lib.HandlerStore) []RouteConfig {
	...
	for _, path := range []string{
		...
		routes = append(routes, RouteConfig{
			...
			PreCallback: createOpenAIListModelsPreCallback(handlerStore),
		})
	}
...
// createOpenAIListModelsPreCallback creates a combined pre-callback for OpenAI list models
// that handles both Azure endpoint preprocessing and query parameter extraction
func createOpenAIListModelsPreCallback(handlerStore lib.HandlerStore) PreRequestCallback {
