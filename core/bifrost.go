...
func (bifrost *Bifrost) ListAllModels(ctx context.Context, request *schemas.BifrostListModelsRequest) (*schemas.BifrostListModelsResponse, *schemas.BifrostError) {
	...
	const maxIterations = 20 // Safety limit per provider
	...
	for _, providerKey := range providerKeys {
		...
		for {
			...
			if iterations > maxIterations {
				bifrost.logger.Warn(fmt.Sprintf("reached maximum pagination requests (%d) for provider %s", maxIterations, providerKey))
				break
			}
