...
import { useCreatePluginMutation, useUpdatePluginMutation } from '@/lib/store'
...
const pluginFormSchema = z.object({
  name: z.string().min(1, 'Plugin name is required').regex(/^[a-z0-9-_]+$/, 'Plugin name must contain only lowercase letters, numbers, hyphens, and underscores'),
  path: z.string().min(1, 'Plugin path/URL is required').refine(
    (val) => {
  ...
  const onSubmit = async (data: PluginFormData) => {
    try {
      ...
      const errorMessage = error && typeof error === 'object' && 'data' in error
        ? (error.data as any)?.error || (isEditMode ? 'Failed to update plugin' : 'Failed to create plugin')
        : isEditMode ? 'Failed to update plugin' : 'Failed to create plugin'
